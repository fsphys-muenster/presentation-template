...
\documentclass[belltower, wide]{beamer}
\usepackage{wwustyle}
...
\title{Hier steht der Titel\\ der Pr√§sentation}
\subtitle{Dies ist der Unteritel}
\author{Name: der Referentin / des Referenten}
% Der hier auskommentierte \title{} unterlegt die weiﬂe Schrift
...
%\title{\bgbox{Hier steht der Titel} \\\bgbox{der Pr√§sentation}}
%\subtitle{\bgbox{Optionen f√ºr \texttt{\string\documentclass}: inverse, wedge, Verwendung von \texttt{\string\bgbox}}}
%\author{\bgbox{Name: der Referentin / des Referenten}}

\institutelogo{\vbox{\hsize=35mm Freiraum f√ºr Sekund√§rlogo(s) / Name \\ des Fachbereichs, Instituts oder SFBs}}
...
\begin{frame}
  \frametitle{Frame 1: Folientitel sollten kurz und pr\"agnant sein}
  {Gestufte Listen}
  \pause
  \begin{itemize}
...
\begin{frame}
  \frametitle{Frame 2: Folientitel sollten kurz und pr\"agnant sein}
  ...
  Auch ``enumerate''-Umgebungen werden von der Stilvorlage √ºberschrieben:
  \end{block}
  \pause
  \begin{enumerate}
    \item So wird sichergestellt,
        \item dass alle Elemente der Pr√§sentation
...
\begin{frame}
  \frametitle{Frame 3: Folientitel sollten kurz und pr\"agnant sein}
  \begin{block}{Hervorhebungen}
    \textbf{Wenn man Dinge hervorheben m\"ochte, nutzt man entweder Fettdruck,}
    \textit{kursive Schrift} \alert{ oder das Schl\"usselwort ``alert''}.
  Auch ``enumerate''-Umgebungen werden von der Stilvorlage √ºberschrieben:
